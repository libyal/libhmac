/*
 * Python definition of the libhmac SHA1 functions
 *
...
/* Calculates the SHA1 of the data
 * Returns 1 if successful or -1 on error
 */
PyObject *pyhmac_sha1_calculate(
           ...
           PyObject *keywords )
{
	...
	static char *function       = "pyhmac_sha1_calculate";
	...
	if( result != 1 )
	{
		pyhmac_error_raise(
		 error,
		 PyExc_IOError,
		 "%s: unable to calculate SHA1 from data.",
		 function );
 ...
 * Returns 1 if successful or -1 on error
 */
PyObject *pyhmac_sha1_calculate_hmac(
           PyObject *self PYHMAC_ATTRIBUTE_UNUSED,
           PyObject *arguments,
           PyObject *keywords )
{
	uint8_t sha1_hmac[ LIBHMAC_SHA1_HASH_SIZE ];
	...
	if( result != 1 )
	{
		pyhmac_error_raise(
		 error,
		 PyExc_IOError,
		 "%s: unable to calculate SHA1 HMAC from data.",
		 function );
	...
	if( bytes_object == NULL )
	{
		PyErr_Format(
		 PyExc_IOError,
		 "%s: unable to convert SHA1 HMAC into bytes object.",
		 function );
