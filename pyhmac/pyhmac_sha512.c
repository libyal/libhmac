/*
 * Python definition of the libhmac SHA512 functions
 *
...
/* Calculates the SHA512 of the data
 * Returns 1 if successful or -1 on error
 */
PyObject *pyhmac_sha512_calculate(
           ...
           PyObject *keywords )
{
	...
	static char *function       = "pyhmac_sha512_calculate";
	...
	if( result != 1 )
	{
		pyhmac_error_raise(
		 error,
		 PyExc_IOError,
		 "%s: unable to calculate SHA512 from data.",
		 function );
 ...
 * Returns 1 if successful or -1 on error
 */
PyObject *pyhmac_sha512_calculate_hmac(
           PyObject *self PYHMAC_ATTRIBUTE_UNUSED,
           PyObject *arguments,
           PyObject *keywords )
{
	uint8_t sha512_hmac[ LIBHMAC_SHA512_HASH_SIZE ];
	...
	if( result != 1 )
	{
		pyhmac_error_raise(
		 error,
		 PyExc_IOError,
		 "%s: unable to calculate SHA512 HMAC from data.",
		 function );
	...
	if( bytes_object == NULL )
	{
		PyErr_Format(
		 PyExc_IOError,
		 "%s: unable to convert SHA512 HMAC into bytes object.",
		 function );
