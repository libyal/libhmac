/*
 * Python definition of the libhmac MD5 functions
 *
...
/* Calculates the MD5 of the data
 * Returns 1 if successful or -1 on error
 */
PyObject *pyhmac_md5_calculate(
           ...
           PyObject *keywords )
{
	...
	static char *function       = "pyhmac_md5_calculate";
	...
	if( result != 1 )
	{
		pyhmac_error_raise(
		 error,
		 PyExc_IOError,
		 "%s: unable to calculate MD5 from data.",
		 function );
 ...
 * Returns 1 if successful or -1 on error
 */
PyObject *pyhmac_md5_calculate_hmac(
           PyObject *self PYHMAC_ATTRIBUTE_UNUSED,
           PyObject *arguments,
           PyObject *keywords )
{
	uint8_t md5_hmac[ LIBHMAC_MD5_HASH_SIZE ];
	...
	if( result != 1 )
	{
		pyhmac_error_raise(
		 error,
		 PyExc_IOError,
		 "%s: unable to calculate MD5 HMAC from data.",
		 function );
	...
	if( bytes_object == NULL )
	{
		PyErr_Format(
		 PyExc_IOError,
		 "%s: unable to convert MD5 HMAC into bytes object.",
		 function );
