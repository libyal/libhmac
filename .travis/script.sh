...
then
	docker run -t -v "${PWD}:/libhmac" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libhmac && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libhmac;
