...
test_callback()
{
	...
	run_test_with_input_and_arguments "${TEST_EXECUTABLE}" -d sha256 ${INPUT_FILE} | ${GREP} "SHA256" | ${SED} 's/^[^:]*[:][\t][\t]*//' > ${TMPDIR}/sha256;
	local RESULT=$?;

	DIGEST_HASH=`cat ${TMPDIR}/sha256`;

	if test ${RESULT} -eq ${EXIT_SUCCESS};
	then
		if test "${PLATFORM}" = "Darwin";
		then
			VERIFICATION_DIGEST_HASH=`sha256 ${INPUT_FILE} | ${SED} 's/[ ][ ]*[^ ][^ ]*$//'`;
		else
	...
	echo -n "Testing ${TEST_PROFILE} -d sha256 of input: ${INPUT_FILE} ";
...
if test "${PLATFORM}" = "Darwin";
then
	assert_availability_binary sha256;
else
